...
# Hyper Parameters
METHOD = "SalNet_Mix_VAE_" + str(args.beta)
...
def main():
    ...
    for episode in range(EPISODE):   
        ...
        support_mix_features = support_mix_features.view(CLASS_NUM*SUPPORT_NUM_PER_CLASS,64,19*19)
        ...
        loss = loss_rn + BETA2*loss_vae #mse(relations,one_hot_labels)
        ...
        if np.mod(episode,2500)==0:
            ...
            for i in range(TEST_EPISODE):
                ...
                for query_img,query_sal,query_labels in query_dataloader:
                    ...
                    support_fore_features = supp_fore_sigma*torch.rand(supp_fore_sigma.size()).cuda() + supp_fore_mu
                    support_back_features = supp_back_sigma*torch.rand(supp_back_sigma.size()).cuda() + supp_back_mu
                    support_mix_features = mixture_network(0.2*support_fore_features + 0.8*support_back_features).view(CLASS_NUM*SUPPORT_NUM_PER_CLASS,64,19*19)
