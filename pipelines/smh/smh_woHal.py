...
def main():
    ...
    for episode in range(EPISODE):   
        ...
        # calculate features      
        s_fore_feat = foreground_encoder(Variable(support_img*support_sal).cuda(GPU)).view(CLASS_NUM*SUPPORT_NUM_PER_CLASS,64,19,19) #obtain foreground features
        s_back_feat = background_encoder(Variable(support_img*(1-support_sal)).cuda(GPU)).view(CLASS_NUM*SUPPORT_NUM_PER_CLASS,64,19,19) #obtain background features
        ...
        s_mix_feat = mixture_network(args.gamma*s_fore_feat +  (1-args.gamma)*s_back_feat).view(CLASS_NUM*SUPPORT_NUM_PER_CLASS,64,19**2)
        
        #s_mix_feat = mixture_network((s_fore_feat + s_back_feat).view(CLASS_NUM*(SUPPORT_NUM_PER_CLASS**2),64,19,19)).view(CLASS_NUM,SUPPORT_NUM_PER_CLASS,-1,64,19**2).sum(2).sum(1)
        ...
        so_s_feat = Variable(torch.Tensor(CLASS_NUM*SUPPORT_NUM_PER_CLASS, 1, 64, 64)).cuda(GPU)
        ...
        so_s_feat = so_s_feat.view(CLASS_NUM, -1, 1, 64, 64).mean(1)
        ...
        if np.mod(episode,2500)==0:
            ...
            for i in range(TEST_EPISODE):
                ...
                for query_img,query_sal,query_labels in query_dataloader:
                    ...
                    s_fore_feat = s_fore_feat#.unsqueeze(2).repeat(1,1,SUPPORT_NUM_PER_CLASS,1,1,1)
                    s_back_feat = s_back_feat#.unsqueeze(1).repeat(1,SUPPORT_NUM_PER_CLASS,1,1,1,1)
                    s_mix_feat = mixture_network(args.gamma*s_fore_feat+(1-args.gamma)*s_back_feat).view(-1,64,19**2)#.view(CLASS_NUM*(SUPPORT_NUM_PER_CLASS**2),64,19,19)).view(CLASS_NUM,SUPPORT_NUM_PER_CLASS,-1,64,19**2).sum(2).sum(1)
                    q_mix_feat = mixture_network(args.gamma*q_fore_feat+(1-args.gamma)*q_back_feat).view(-1,64,19**2)
                    so_s_feat = Variable(torch.Tensor(CLASS_NUM*SUPPORT_NUM_PER_CLASS, 1, 64, 64)).cuda(GPU)
